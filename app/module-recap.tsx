...
import {ArrowLeft,ArrowRight,BookOpen,CheckCircle2,ExternalLink,List,Printer,X} from 'lucide-react';
  ...
  return <div className="recap-spread">
    ...
    <section className="recap-paper recap-connections" aria-label="Key relationships">
      ...
      <div className={`recap-visual ${chapter.visual.kind}`}>
        {chapter.visual.kind==='flow'?<ol>{chapter.visual.lines.map(line=><li key={line}>{line}</li>)}</ol>:<ul>{chapter.visual.lines.map(line=><li key={line}>{line}</li>)}</ul>}
      </div>
  ...
  return <section ref={dialog} className="module-recap" role="dialog" aria-modal="true" aria-labelledby="recap-book-title"
    ...
    <div className="recap-screen">
      ...
      <div className="recap-reading-area" ref={readingArea}>
        ...
        <Spread key={chapter.id} chapter={chapter}/>
        <details className="recap-sources" key={chapter.id+'-sources'}><summary>Source videos · {chapter.lessons.length} lessons <span>Reading this recap does not mark videos or quizzes complete.</span></summary>
          <ul>{chapter.lessons.map(lesson=><li key={lesson.id}>
            <div><a href={lesson.url} target="_blank" rel="noopener noreferrer">L{String(lesson.number).padStart(2,'0')} · {lesson.title}<ExternalLink size={15}/></a>
