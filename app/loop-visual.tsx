...
export default function LoopVisual(){
  ...
  return <details className="loop-explorer"><summary>See the fault-current path</summary><p>Predict first: if the circuit resistance increases at the same source voltage, will the prospective fault current rise or fall?</p><label>Circuit R1 + R2: <strong>{circuit.toFixed(2)} Ω</strong><input type="range" aria-label="Circuit line and protective conductor resistance" min=".1" max="2" step=".1" value={circuit} onChange={event=>setCircuit(Number(event.target.value))}/></label><div className="button-row"><button type="button" className="secondary-button" aria-pressed={open} onClick={()=>setOpen(value=>!value)}>{open?'Restore the virtual CPC':'Open the virtual CPC'}</button><button type="button" className="secondary-button" aria-pressed={motion} onClick={()=>setMotion(value=>!value)}>{motion?'Pause flow':'Animate flow'}</button></div>
    <svg className={`loop-diagram ${motion&&!open?'flowing':''}`} viewBox="0 0 640 290" role="img" aria-label={open?'Open protective path: no current in the idealised intended loop; accessible metal may remain hazardous.':`Complete fault path from source along line to the enclosure and back through CPC. Approximate prospective current ${result.current.toFixed(0)} amperes.`}>
      <path d="M95 70 H540 V220 H95 Z" fill="none" stroke="var(--control-border)" strokeWidth="6"/>
      <path className="loop-current" d={open?'M95 70 H540 V220 H370':'M95 70 H540 V220 H95 V70'} fill="none" stroke="var(--text-accent)" strokeWidth="4"/>
      <rect x="50" y="100" width="90" height="85" rx="8" fill="var(--surface-inverse)"/><text x="95" y="132" textAnchor="middle" fill="var(--text-on-inverse)">Source</text><text x="95" y="160" textAnchor="middle" fill="var(--text-on-inverse)">230 V</text>
      <rect x="235" y="43" width="135" height="55" rx="8" fill="var(--surface-muted)" stroke="var(--control-border)"/><text x="302" y="76" textAnchor="middle" fill="var(--ink)">Protection</text>
      <rect x="480" y="110" width="125" height="65" rx="8" fill="var(--surface-warning)" stroke="var(--text-warning)"/><text x="542" y="136" textAnchor="middle" fill="var(--text-warning)">Line-to-case</text><text x="542" y="158" textAnchor="middle" fill="var(--text-warning)">fault</text>
      {open&&<><path d="M270 220 H355" stroke="var(--surface-soft)" strokeWidth="12"/><path d="M270 220 L350 188" stroke="var(--text-danger)" strokeWidth="5"/><text x="310" y="270" textAnchor="middle" fill="var(--text-danger)">CPC open</text></>}
      <text x="420" y="52" textAnchor="middle" fill="var(--ink)">Line →</text><text x="300" y="205" textAnchor="middle" fill="var(--ink)">← CPC return</text><text x="100" y="260" textAnchor="middle" fill="var(--ink)">Ze = 0.30 Ω</text>
    </svg>
