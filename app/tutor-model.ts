...
export function isProgressBackup(value: unknown, lessonIds: Set<string>): boolean {
  ...
  return Array.isArray(input.completedLessonIds) && typeof input.activeLessonId === 'string'
    && lessonIds.has(input.activeLessonId) && (input.schemaVersion === undefined || (typeof input.schemaVersion === 'number' && Number.isInteger(input.schemaVersion) && input.schemaVersion >= 1 && input.schemaVersion <= 9));
}
