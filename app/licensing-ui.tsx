...
import course, { isRequiredLesson } from './course-curriculum';
...
export function LicensingStageGuide({moduleId}:{moduleId:string}) {
  ...
  return <details className="licensing-stage-guide"><summary><Info size={18}/><span><small>{stage.path} · Stage {stage.stageNumber}</small><strong>About this stage</strong></span><ChevronDown size={18}/></summary><div className="licensing-stage-guide-body"><h2>{stage.title}</h2><p>{stage.description}</p>
    ...
    {moduleId==='module-02'&&<p className="licensing-gap"><strong>Kenyan bridge slot:</strong> add a current Kenyan video covering EPRA C2 scope, statutory responsibilities, supply arrangements and the locally applicable KS 662 requirements. The international videos teach the electrical relationships; they do not replace Kenyan legal requirements.</p>}
    {moduleId==='module-03'&&<p className="licensing-gap"><strong>Kenyan bridge slot:</strong> add a local demonstration of conductor identification, cable markings and common Kenyan products. Always prove conductor identity; colour is an aid, not test evidence.</p>}
    ...
    {moduleId==='module-06'&&<p className="licensing-gap"><strong>Kenyan design bridge:</strong> add one complete C2 worked design using the current Kenyan supply values, KS 662 tables/limits and EPRA expectations, from load schedule through final checks.</p>}
    ...
    {moduleId==='module-08'&&<p className="licensing-gap"><strong>Kenyan handover bridge:</strong> add a current local walkthrough of the inspection schedule, test record, certificate, notices and submission/handover process expected for a Kenyan C2 installation.</p>}
    {moduleId==='module-09'&&<p><strong>Use the same method in every case:</strong> define the symptom, make the installation safe, gather evidence, divide the circuit logically, select the least hazardous discriminating test, repair, then repeat the required verification—not only the test that found the fault.</p>}
...
export function LicensingOverview({initialPath=null,completed,onLesson}:Props) {
  ...
  return <section className="licensing-overview" aria-label="Licensing pathways"><h2>Your learning progression</h2><p>Study the C2 foundation, extend it through C1, then explore specialist systems. Progress records watched videos. These study records are not an EPRA licence or proof of practical competence.</p><div className="licensing-paths">{(['C2','C1','Professional'] as LicensingPath[]).map(path=>{
    const modules=course.modules.filter(m=>m.path===path),allLessons=modules.flatMap(m=>m.lessons),lessons=allLessons.filter(l=>isRequiredLesson(l.id));
    const watched=lessons.filter(l=>done.has(l.id)).length;
    return <article key={path}><span className="eyebrow">{path==='Professional'?'Optional enrichment':path+' core'}</span><h3>{pathLabels[path]}</h3><p>{path==='C2'?'Principles → safety → circuits → protection → design → verification → diagnosis':path==='C1'?'Three-phase → design → power factor → motors → verification':'Specialist cables, lighting, automation, energy systems and professional practice'}</p><p>{watched}/{lessons.length} core and applied videos watched{allLessons.length>lessons.length?` · ${allLessons.length-lessons.length} optional reinforcement/deep-practice videos`:''}</p><button className="secondary-button" onClick={()=>onLesson(lessons.find(l=>!done.has(l.id))?.id??lessons[0].id)}>Open {path} pathway</button>{path!=='Professional'&&<><button className="secondary-button" onClick={()=>setExam(exam===path?null:path)}>EPRA {path} preparation</button><p className="licensing-milestone">{watched===lessons.length?path+' core videos watched':path+' core videos in progress'}</p></>}</article>;
  })}</div>
