...
export async function GET() {
  ...
  try { const user = await requireCourseUser(); return Response.json(await readReaderState(user.id), { headers: privateHeaders }); }
...
export async function PATCH(request: Request) {
  ...
  try { const user = await requireCourseUser(); return Response.json(await updateReaderState(user.id, command), { headers: privateHeaders }); }
  catch (error) {
