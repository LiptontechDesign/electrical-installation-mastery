...
export type LessonWorkspaceMode = 'guide' | 'standards';
...
export default function LessonOverview({ lessonId, guide, watched, learningText, mode = 'guide', onModeChange, onLesson, onRead }: {
  lessonId: string; guide: LessonGuide; watched: boolean; learningText: string;
  mode?: LessonWorkspaceMode; onModeChange?: (mode: LessonWorkspaceMode) => void;
  ...
  const activeMode: LessonWorkspaceMode = standardsSection && mode === 'standards' ? 'standards' : 'guide';
  const selectMode = (nextMode: LessonWorkspaceMode, focus = false) => {
    onModeChange?.(nextMode);
    if (focus) window.requestAnimationFrame(() => document.getElementById(`${nextMode}-tab-${lessonId}`)?.focus());
  };
  const onModeKeyDown = (event: KeyboardEvent<HTMLButtonElement>) => {
    if (!standardsSection || !['ArrowLeft','ArrowRight','Home','End'].includes(event.key)) return;
    event.preventDefault();
    selectMode(event.key === 'ArrowLeft' || event.key === 'Home' ? 'guide' : 'standards', true);
  };
  return <div className="lesson-learning-workspace">
    {standardsSection&&<div className="lesson-learning-switch" role="tablist" aria-label="Choose lesson content">
      <button id={`guide-tab-${lessonId}`} type="button" role="tab" aria-selected={activeMode==='guide'} aria-controls={`guide-panel-${lessonId}`} tabIndex={activeMode==='guide'?0:-1} onClick={()=>selectMode('guide')} onKeyDown={onModeKeyDown}>Lesson guide</button>
      <button id={`standards-tab-${lessonId}`} type="button" role="tab" aria-selected={activeMode==='standards'} aria-controls={`standards-panel-${lessonId}`} tabIndex={activeMode==='standards'?0:-1} onClick={()=>selectMode('standards')} onKeyDown={onModeKeyDown}>Standards companion</button>
    </div>}
    <section id={`${activeMode}-panel-${lessonId}`} className={`lesson-workspace-panel ${activeMode}-mode`} role={standardsSection?'tabpanel':undefined} aria-labelledby={standardsSection?`${activeMode}-tab-${lessonId}`:undefined}>
      {activeMode==='guide'?<div className="overview-flow">
        ...
        {sourceClarifications[lessonId]&&<aside className="course-source-context"><strong>Source context</strong><p><LearningText text={sourceClarifications[lessonId]}/></p></aside>}
        ...
        <CourseBridge lessonId={lessonId}/>
      ...
      </div>:<div className="standards-workspace">
        <OverviewReader key={standardsSection!.id} section={standardsSection!} terms={overviewData.terms} sources={overviewData.sources} formulas={overviewData.formulas} onLesson={onLesson} onRead={onRead}/>
