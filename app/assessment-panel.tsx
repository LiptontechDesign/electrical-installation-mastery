...
import { ArrowRight, Award, BookOpen, Check, ChevronLeft, ChevronRight, Circle, HardHat, Lightbulb, ListChecks, MessageCircleQuestion, RotateCcw, ShieldAlert, ShieldCheck, Target } from 'lucide-react';
      ...
      {mode === 'cards' && card && (
        <div className="flashcard-workspace">
          ...
          <button type="button" className={`learning-flashcard ${cardRevealed ? 'revealed' : ''}`} onClick={() => setCardRevealed((current) => !current)} aria-label={cardRevealed ? 'Hide answer' : 'Reveal answer'}>
            <div className="learning-item-labels"><span className="flashcard-side-label">{cardRevealed ? 'Direct answer' : 'Recall before revealing'}</span><span className="learning-kind">{card.kind}</span></div>
            <span className="assessment-lesson-source">From lesson: {card.lessonTitle}</span>
            <h3>{cardRevealed ? card.back : card.front}</h3>
            {cardRevealed ? <>
              <p className="answer-explanation">{card.explanation}</p>
              ...
              <div className="flashcard-teaching-grid">
                <div className="why-it-matters"><Lightbulb size={19} /><p><strong>Why it matters</strong>{card.whyItMatters}</p></div>
                <div className="job-application"><HardHat size={19} /><p><strong>Use it on the job</strong>{card.application}</p></div>
              </div>
              <div className="misconception-check"><ShieldAlert size={18} /><p><strong>Avoid this misunderstanding</strong>{card.misconception}</p></div>
              {card.kenyaNote && <div className="kenya-card-note"><ShieldCheck size={18} /><p>{card.kenyaNote}</p></div>}
            </> : <p className="flip-instruction">Pause. Give a direct answer first, explain why, then add one site example. Reveal only after you have attempted all three.</p>}
          </button>
          <div className="flashcard-actions">
            ...
            {cardRevealed ? <div className="card-rating"><button type="button" className="again" onClick={() => rateCard(false)}><RotateCcw size={18} /> Need another look</button><button type="button" className="got-it" onClick={() => rateCard(true)}><Check size={18} /> I can explain it</button></div> : <button type="button" className="reveal-card" onClick={() => setCardRevealed(true)}>Reveal teaching answer</button>}
      ...
      {mode === 'quiz' && !finished && question && (
        <div className="assessment-quiz">
          ...
          {revealed && <div className={answers[questionIndex] === question.answer ? 'assessment-feedback correct' : 'assessment-feedback'} role="status" aria-live="polite">
            ...
            <div>
              <strong>{answers[questionIndex] === question.answer ? 'Correct—your answer matches the question.' : 'Not yet—compare your choice with the exact question.'}</strong>
              <div className="feedback-direct-answer"><span>Best answer</span><p>{question.options[question.answer]}</p></div>
              {answers[questionIndex] !== question.answer && <p className="selected-answer-feedback"><b>Why your choice missed:</b> {question.optionFeedback[answers[questionIndex]]}</p>}
              <div className="feedback-reasoning"><span>Why it is correct</span><p>{question.explanation}</p></div>
              <div className="feedback-transfer"><HardHat size={17} /><p><strong>Use it on the job</strong>{question.jobConnection}</p></div>
              <small><b>One sentence to retain:</b> {question.takeaway}</small>
            </div>
          </div>}
