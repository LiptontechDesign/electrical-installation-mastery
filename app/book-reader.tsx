...
import { ArrowLeft, ArrowRight, Bookmark, BookOpen, ChevronLeft, ChevronRight, Cloud, FlaskConical, List, Search, X, ZoomIn, ZoomOut } from 'lucide-react';
...
function Bookshelf({ state, ready, onOpen }: { state: ReaderState; ready: boolean; onOpen: (id: BookId, page: number) => void }) {
  ...
  })}</div><p className="books-source-note">Historical textbooks · use the examples to understand the concepts, and check current requirements before applying them.</p></div>;
}
...
function ReaderWorkspace({ initialReading }: { initialReading?: Reading }) {
  ...
  const [view, setView] = useState<'books' | 'simulations'>('books');
  const [mode, setMode] = useState<'page' | 'text'>('page');
  const [focusedReading, setFocusedReading] = useState(initialReading);
  ...
  const pageTopic = selected && simulationForPage(selected.id, selected.page);
  ...
  const open = (id: BookId, page: number) => { setSelected({ id, page }); setSidebar(false); setQuery(''); setZoom(1); setMode('page'); setFigureId(null); setFocusedReading(undefined); setView('books'); lastRendered.current = ''; };
...
export default function BookReader({ initialReading, onClose }: { initialReading?: Reading; onClose: () => void }) {
  ...
  return <dialog ref={dialog} className="book-reader-dialog" aria-labelledby="book-reader-title" onCancel={onClose}><header className="book-reader-header"><div><BookOpen size={22} /><h2 id="book-reader-title">My books</h2></div><button type="button" aria-label="Close book reader" onClick={onClose}><X size={23} /></button></header><div className="reader-content">{session?.authenticated ? <ReaderWorkspace initialReading={initialReading} /> : <div className="reader-message">{error ? <><p role="alert">{error}</p><button type="button" onClick={() => { setError(''); setRetry(value => value + 1); }}>Retry opening books</button></> : <p role="status">Opening your books…</p>}</div>}</div></dialog>;
}
