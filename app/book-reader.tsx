...
import { ArrowLeft, ArrowRight, Bookmark, BookOpen, Check, ChevronLeft, ChevronRight, Cloud, List, LockKeyhole, LogOut, Search, X, ZoomIn, ZoomOut } from 'lucide-react';
...
function ReaderWorkspace({ initialReading, onLocked }: { initialReading?: Reading; onLocked: () => void }) {
...
export default function BookReader({ initialReading, onClose }: { initialReading?: Reading; onClose: () => void }) {
  ...
  useEffect(() => {
    let active = true;
    requestJson<Session>('/api/reader/session').then(value => { if (active) { setSession(value); setError(''); } }).catch(() => { if (active) setError('Could not connect to the private reader.'); });
    return () => { active = false; };
  }, [retry]);
  const unlock = async (event: FormEvent) => {
    event.preventDefault(); setBusy(true); setError('');
    try { await requestJson('/api/reader/session', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ key }) }); setKey(''); setSession({ configured: true, authenticated: true }); }
    catch (error) { setError(error instanceof Error ? error.message : 'Could not unlock your books.'); }
    finally { setBusy(false); }
  };
  return <dialog ref={dialog} className="book-reader-dialog" aria-labelledby="book-reader-title" onCancel={onClose}><header className="book-reader-header"><div><BookOpen size={22} /><h2 id="book-reader-title">My books</h2></div><button type="button" aria-label="Close book reader" onClick={onClose}><X size={23} /></button></header>{session?.authenticated ? <ReaderWorkspace initialReading={initialReading} onLocked={() => setSession({ configured: true, authenticated: false })} /> : <div className="reader-unlock"><LockKeyhole size={29} /><h3>{session?.configured ? 'Your private reading space.' : session ? 'Your books are being connected.' : 'Opening your books…'}</h3>{session?.configured ? <><p>Unlock with your reader key. Use the same key on another device to continue from your saved page.</p><form onSubmit={unlock}><label>Private reader key<input type="password" autoComplete="current-password" value={key} onChange={event => setKey(event.target.value)} required spellCheck={false} /></label><button className="reader-primary" type="submit" disabled={busy || !key.trim()}>{busy ? 'Unlocking…' : 'Unlock books'} <ArrowRight size={17} /></button></form><small>This device stays unlocked for 30 days, or until you lock it.</small></> : session && <p>The two books will appear here after private Vercel storage is connected and the upload is complete.</p>}{error && <p role="alert">{error}</p>}{(!session || !session.configured) && <button type="button" onClick={() => setRetry(value => value + 1)}>Check connection</button>}</div>}</dialog>;
}
