...
let tree;
await act(async () => { tree = create(h(AssessmentPanel, { ...props, bestScore: 9, mode: 'quiz' })); });
...
for(const value of ['Latest 1/2','Best 2/2','4 recorded attempts','Passed previously'])assert.ok(resultText.includes(value),`Result history includes “${value}”`);
assert.ok(button('Try again'),'Completed quizzes can be repeated');
assert.ok(button('Continue learning'),'A lower retry does not relock a previously passed checkpoint');
await act(async () => button('Try again').props.onClick());
await act(async () => tree.root.findAllByProps({className:'assessment-options'})[0].findAllByType('button')[0].props.onClick());
await act(async () => button('Restart').props.onClick());
await act(async () => tree.root.findAllByProps({className:'assessment-options'})[0].findAllByType('button')[0].props.onClick());
await act(async () => button('Next question').props.onClick());
await act(async () => tree.root.findAllByProps({className:'assessment-options'})[0].findAllByType('button')[0].props.onClick());
await act(async () => button('See results').props.onClick());
assert.equal(attempts.length,2,'Restarting an unfinished quiz does not save an attempt');
assert.ok(text(tree.toJSON()).includes('5 recorded attempts'),'Only completed submissions increase the attempt count');
await act(async () => tree.update(h(AssessmentPanel, { ...props, mode: 'cards' })));
